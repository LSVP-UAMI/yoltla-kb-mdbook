...
*Certbot* es una herramienta para generar certificados 
de forma cencilla. *Certbot* usa al proveedor de 
certificados *Letsencrypt*

La instalación de certbot se realizo dentro de un contenedor 
usando la imagen oficial de docker (https://hub.docker.com/r/certbot/certbot/[Imagen]).

==== Docker compose

Para crear un contenedor de *Certbot* en docker compose se agrega lo siguiente 
al archivo _docker-compose.yml_
...
Es necesario montar los volumenes para _/var/www/certbot_ y _/etc/letsencrypt_ ya que la 
información que se encuentra en estos directorios debe ser compartida con el contenedor 
de *Nginx*

Para montar los directorios _/var/www/certbot_ y _/etc/letsencrypt_ de *Certbot* en el 
...
_/etc/nginx/conf.d/default.conf_
...
Para que *Certbot* pueda generar los certificados es necesario que *Nginx* este en ejecución
con las configuraciones previamente mencionadas.

Antes de generar los certificados es conveniente probar que todo este configurado de una forma 
adecuada para esto podemos usar el comando 
