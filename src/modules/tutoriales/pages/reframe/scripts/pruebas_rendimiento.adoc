...
Para obtener más información, consulte la sección https://reframe-hpc.readthedocs.io/en/stable/tutorial_basics.html#writing-a-performance-test[Writing A Performance Test] de la documentación oficial de ReFrame.


[#ejemplos]
== Ejemplos

El siguiente script de reframe ejecuta una prueba del microbenchmark https://www.cs.virginia.edu/stream/ref.html[STREAM]:
...
En este ejemplo, extraemos cuatro variables de rendimiento, que son los valores de ancho de banda de la memoria para cada uno de los subpuntos de referencia: `Copy`, `Scale`, `Add` y `Tríad` de STREAM, donde cada una de las funciones de rendimiento utiliza la función de utilidad xref:reframe/scripts/pruebas_sanidad#extractsingle[`extractsingle()`]. Para cada uno de los subpuntos de referencia, se define una https://docs.python.org/3/library/re.html[expresión regular] de Python para extraer la columna "Best Rate MB/s" de la salida (ver más abajo) y la convertimos en un valor flotante.
...
Durante la etapa de rendimiento, los elementos de `reference`, excepto la unidad, se pasan a la función https://reframe-hpc.readthedocs.io/en/stable/deferrable_functions_reference.html#reframe.utility.sanity.assert_reference[`assert_reference()`] junto con el valor de rendimiento obtenido para evaluar realmente si la prueba pasa la verificación de rendimiento o no.
...
Para obtener más información, consulte la sección xref:reframe/anexos/archivo_configuracion.adoc#configuracion_logging[Configuración de Logging].
...
Este ejemplo usa otros elementos de ReFrame, sugerimos consultar la sección xref:reframe/scripts/parametros.adoc[Parámetros] y xref:reframe/anexos/topologia.adoc[Topología].