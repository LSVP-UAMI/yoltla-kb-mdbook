...
1. Directorio de la prueba

2. Prueba de ReFrame

3. Directorio donde se guardarán los logs de la prueba

4. En este directorio deben colocarse todos los recursos necesarios para la prueba:
...
| Opción | Descripción |
|:------:|:------------|
| -c, --checkpath=PATH | Una ruta del sistema de archivos donde ReFrame debería buscar pruebas. <br> PATH puede ser un directorio o un solo archivo de prueba. |
...
En este punto, la prueba se encuentra en el sistema de colas del cluster Yoltla esperando 
por los recursos necesarios para ser ejecutada.
...
El criterio que utiliza ReFrame para determinar si una prueba concluyó con éxito, es la 
prueba de sanidad (*sanity check*). Por lo que todas las pruebas de ReFrame deben tener 
una función con el decorador `@sanity_function`.

Para obtener más información, consulte la sección [Pruebas de sanidad](../scripts/pruebas_sanidad.md).
```
...
│   │           └── builtin-gcc-7.2.0
│   │               └── HelloTest
│   │                   ├── rfm_HelloTest_build.sh      (3)
│   │                   ├── rfm_HelloTest_build.out     (4)
│   │                   ├── rfm_HelloTest_build.err     (5)
│   │                   └── rfm_HelloTest_job.sh        (6)
│   │                   ├── rfm_HelloTest_job.out       (7)
│   │                   ├── rfm_HelloTest_job.err       (8)
│   ├── __pycache__
...
1. Directorio que almacena los logs de la prueba

2. Directorio que almacena los scripts y archivos de salida/error de la prueba

3. Script para compilar el archivo fuente

4. Salida estándar de la compilación

5. Error estándar de la compilación

6. Script para ejecutar la prueba

7. Salida estándar de la ejecución

8. Error estándar de la ejecución
...
Como puede observar la salida de esta prueba es muy similar a la vista en el ejemplo 
anterior. La prueba terminó con éxito y no parece arrojarnos niguna información 
adicional, por lo que continuaremos con el análisis de su estructura de directorios.
...
1. Directorio en el que se almacenan los resultados de las pruebas de rendimiento

2. Archivo con los resultados de la prueba de rendimiento

Es muy similar al visto en el ejemplo anterior, pero nos encontramos con un nuevo 
...
múltiples pruebas con un solo comando. ReFrame implementa diferentes mecanismos que nos 
permiten realizar esta función, y uno de estos es el uso de etiquetas (`tags`). Las 
...
En este ejemplo tenemos dos pruebas de ReFrame, con sus respectivos recursos, en un 
mismo directorio, y nos interesa lanzar ambas pruebas. En primer instancia, podemos 
pensar en simplemente lanzar primero la prueba *hello_test.py* y posteriormente la 
prueba *hola_test.py*, lo cual es una solución válida, sin embargo, si nos encontraramos 
en un escenario donde fuera necesario lanzar 100 pruebas, esto puede volverse complicado. 
Una alternativa a lanzar las pruebas de forma individual es utilizar etiquetas (`tags`).
...
| Opción | Descripción |
|--------|-------------|
| -c, --checkpath=PATH | Una ruta del sistema de archivos donde ReFrame debería buscar pruebas. PATH puede ser un directorio o unsolo archivo de prueba. |
...
Ya hemos visto que gracias a las etiquetas podemos lanzar múltiples pruebas de manera 
simultánea, sin embargo, puede que por diversos motivos no nos interese lanzar las 
pruebas que contengan cierta etiqueta, es decir, queremos exlcuir pruebas.
...
| Opción | Descripción |
|:------:|-------------|
| -c, --checkpath=PATH | Una ruta del sistema de archivos donde ReFrame debería buscar pruebas. PATH puede ser un directorio o un solo archivo de prueba. |
...
Esta opción se puede especificar varias veces, en cuyo caso se excluirán las pruebas 
con cualquiera de las etiquetas especificadas.
```
...
Con el cual lanzará todas las pruebas que tengan la etiqueta \'greetings\', pero no la 
etiqueta \'hello\':
...
En este ejemplo, podemos observar que a diferencia de las salidas vistas anteriormente, 
esta salida nos da información más detallada sobre la ejecución de nuestra prueba, en 
particular nos interesa saber la razón del porque la prueba falló. En este caso, ReFrame 
nos indica que fue debido a un error en la función de sanidad:
...
prueba concluye con éxito, los scripts y archivos de salida/error son movidos al directorio 
*output*, el resto de archivos son eliminados. Sin embargo, cuando la prueba falla, todos
los recursos permanecen en el directorio *stage*:
...
De manera similar al ejemplo anterior, la salida nos da información detallada sobre la 
ejecución de nuestra prueba, en particular nos interesa saber la razón del porque la 
prueba falló. En este caso, ReFrame nos indica que fue debido a un error en la función 
de rendimiento:
