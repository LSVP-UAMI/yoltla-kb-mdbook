...
[NOTE, caption=NOTA]
====
La expresion de evaluación es una https://docs.python.org/3/library/re.html[expresión regular] de Python. Y se usa para buscar una cadena de caracteres en la salida.
====

Toda función de sanidad debe regresar un valor booleano (True). Esto se explicará más adelante, por el momento se destaca el uso de la xref:#funciones_diferibles[función diferible] xref:#assert_found[`sn.assert_found`] que afirma la existencia de una cadena. Esta función se hereda de la clase `sanity`:
...
La prueba ha verificado que se produjo la salida deseada y ha sido exitosa, hemos verificado que tenemos un compilador de C en funcionamiento en nuestro sistema. Si todo ha salido bien, puede verificar el resultado en la carpeta _output_.


[#comprobacion_avanzada]
...
Para obtener más información, consulte la sección https://reframe-hpc.readthedocs.io/en/stable/deferrable_functions_reference.html#deferrable-functions-reference[Deferrable Functions Reference] de la documentación oficial de ReFrame.


[#lista_funciones_utilidades]
...
Plantea:;;   `reframe.core.exceptions.SanityError` – si falla.
...
Regresa:;;          Una lista de valores convertidos extraídos de los grupos de captura especificados en `tag`.
...
ReFrame tiene muchas más funciones útiles, que puede consultar en la sección https://reframe-hpc.readthedocs.io/en/stable/deferrable_functions_reference.html?highlight=sanity.all#list-of-deferrable-functions-and-utilities[List of deferrable functions and utilities] de la documentación oficial de ReFrame.

[NOTE,caption=NOTA]
====
Una función de sanidad *verifica* la presencia de patrones y elementos, por lo tanto debe ser explícita en cuanto al valor que regresa (un booleano). Si la función de sanidad no se puede resolver debido a una sintaxis ambigua, se marcará el error https://reframe-hpc.readthedocs.io/en/stable/exceptions.html#reframe.core.exceptions.ReframeSyntaxError[reframe.core.exceptions.ReframeSyntaxError].
====
...
. Se define una expresión regular que filtre la variable deseada:
+   
----
 r'ENERGY:([ \t]+\S+){10}[ \t]+(?P<energy>\S+)'
----

. Con la función `sn.extractall` se define que se buscará el patrón en la salida estándar y se guardará el valor en la variable 'energy' que es de tipo `float`. La función creará una lista con los valores obtenidos.

. Con la función `sn.avg` se obtiene un promedio de los valores.

. Con la funcion `sn.abs` se obtiene el valor absoluto de la diferencia entre el promedio de la energía obtenida con la energía promedio esperada.

. Se regresa el valor de la función `sn.assert.lt`. La prueba es exitosa si la energía entra en el margen de tolerancia: `energy_diff < energy_tolerance`, en este caso `sn.assert.lt` regresa True, falla la prueba en caso contrario.


[#multiples_pruebas_sanidad]
