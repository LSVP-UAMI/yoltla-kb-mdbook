...
[NOTE,caption=NOTA]
====
Como podrá haber observado, todas las funciones de la clase `Simple_Test` están decoradas. El https://docs.python.org/3/glossary.html#term-decorator[decorador] de cada función le indica a ReFrame en que punto de su xref:reframe/anexos/referencia_api.adoc#pipeline[pipeline] debe ejecutarla. Por ejemplo, en este fragmento de código:
...
Para obtener más información de los decoradores, consulte la sección xref:/reframe/anexos/referencia_api.adoc#test_decorators[Test Decorators].
====
...
Dentro del directorio _reframe_ cree la siguiente estructura de directorios:
...
Posteriormente, cambie al directorio _reframe/hello_world_:
...
y cree el archivo _hello_test.py_:
...
Dentro del directorio _src_, cree el archivo _hello.c_:
...
El contenido del directorio _reframe_ debe ser el siguiente:
----
reframe
├── hello_world
│   ├── hello_test.py
...
|-c, --checkpath=PATH
|Una ruta del sistema de archivos donde ReFrame debería buscar pruebas. +
PATH puede ser un directorio o un solo archivo de prueba.
...
====
El criterio que utiliza ReFrame para determinar si una prueba concluyó con éxito, es la prueba de sanidad (_sanity check_). Por lo que todas las pruebas de ReFrame deben tener una función con el decorador `@sanity_function`.

Para obtener más información, consulte la sección xref:reframe/scripts/pruebas_sanidad.adoc[Pruebas de sanidad].
====
...
Posteriormente, cambie al directorio _reframe/prime_numbers_:
...
y cree el archivo _prime_numbers_test.py_:
...
Dentro del directorio _src_, cree el archivo _prime_numbers.c_:
...
El contenido del directorio _prime_numbers_ debe ser el siguiente:
...
Es muy similar al visto en el ejemplo anterior, pero nos encontramos con un nuevo directorio, el directorio _perflogs_. En este directorio se almacenan los resultados del rendimiento de la prueba:
...
Para obtener más información de las pruebas de rendimiento, consulte la sección xref:reframe/scripts/pruebas_rendimiento.adoc[Pruebas de rendimiento].
...
Dentro del directorio _reframe_ cree la siguiente estructura de directorios y archivos:
...
Posteriormente, cambie al directorio _reframe/tags_:
...
[NOTE,caption=NOTA]
====
TAG se interpreta como una https://docs.python.org/3/library/re.html[expresión regular] de Python; se seleccionarán todas las pruebas que tengan al menos una etiqueta coincidente.
...
Utilizando el mismo directorio _tags_ del ejemplo anterior, cree la siguiente estructura de directorios y archivos:
...
[NOTE,caption=NOTA]
====
TAG se interpreta como una https://docs.python.org/3/library/re.html[expresión regular] de Python; se excluirá cualquier prueba con etiquetas que coincidan con TAG.
...
[NOTE,caption=NOTA]
====
Como ya se mencionó anteriormente, todos los recursos de la prueba se almacenan en el directorio _stage_ de manera temporal, estos recursos incluyen los archivos fuente y compilados, y cualquier otro archivo que se haya generado durante la prueba. Cuando la prueba concluye con éxito, los scripts y archivos de salida/error son movidos al directorio _output_, el resto de archivos son eliminados. Sin embargo, cuando la prueba falla, todos los recursos permanecen en el directorio _stage_:
